...
public class TrailTimeEstimationService {
    ...
    private ForecastObject getDayWithBestWeather(ForecastApiResponse forecastResponse) {
        // sort the weather data ordered by the best weather conditions then get all days with the best condition
        forecastResponse.getDaily().sort(Comparator.comparing(forecast -> weatherOrderOfPreference.indexOf(forecast.getWeather().get(0).getMain())));
        List<ForecastObject> orderedWeatherListOfBestCondition =
                forecastResponse.getDaily().stream()
                                .filter(weather ->
                                        weather.getWeather().get(0).getMain().equals(
                                                forecastResponse.getDaily().get(0).getWeather().get(0).getMain()))
                                .collect(Collectors.toList());
        if (orderedWeatherListOfBestCondition.size() > 1) { // filter out any duplicates if low temp or high wind is present
            orderedWeatherListOfBestCondition = orderedWeatherListOfBestCondition.stream()
                                                                                 .filter(weather ->
                                                                                         weather.getTemp().getDay() > 0 || weather.getWind_speed() > 10)
                                                                                 .collect(Collectors.toList());
        }
        return orderedWeatherListOfBestCondition.size() < 1 ? forecastResponse.getDaily().get(0) : orderedWeatherListOfBestCondition.get(0);
    }

    private LocalDateTime getLatestTimeForHike(double timeEstimate, ForecastObject forecast) {
        LocalDateTime sunsetTime = LocalDateTime.ofEpochSecond(forecast.getSunset(), 0, ZoneOffset.UTC);
